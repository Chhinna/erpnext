...
class AccountsController(TransactionBase):
    ...
    def get_print_settings(self):
        ...
        if items_field and items_field.fieldtype == "Table":
            print_setting_fields += ["compact_item_print",
    ...
    def company_currency(self):
        if not hasattr(self, "__company_currency"):
            self.__company_currency = erpnext.get_company_currency(
                self.company)
    ...
    def ensure_supplier_is_not_blocked(self):
        ...
        is_buying_invoice = self.doctype in [
        ...
        if supplier and supplier_name and supplier.on_hold:
            if (is_buying_invoice and supplier.hold_type in ["All", "Invoices"]) or (
                ...
                if not supplier.release_date or getdate(
                        nowdate()) <= supplier.release_date:
                    frappe.msgprint(
    ...
    def validate(self):
        ...
        if self.doctype in ["Purchase Invoice", "Sales Invoice"]:
            ...
            if cint(
                    self.allocate_advances_automatically) and not cint(
                    self.get(pos_check_field)):
                self.set_advances()
    ...
    def on_trash(self):
        ...
        if frappe.db.get_single_value(
            ...
            frappe.qb.from_(ple).delete().where(
                (ple.voucher_type == self.doctype) & (
                    ple.voucher_no == self.name)).run()
            frappe.db.sql(
        ...
        if (
            ...
            frappe.throw(
                _("To Date cannot be before From Date")(
                    title=_("Invalid Auto Repeat Date")))

    def validate_deferred_start_and_end_date(self):
        for d in self.items:
            if d.get("enable_deferred_revenue") or d.get(
                    "enable_deferred_expense"):
                if not (d.service_start_date and d.service_end_date):
                    ...
                    frappe.throw(
                        _("Row #{0}: Service End Date cannot be before Invoice Posting Date").format(
                            d.idx))
    ...
    def before_print(self, settings=None):
        if self.doctype in [
            ...
            if self.get("discount_amount") and hasattr(
                    self, "taxes") and not len(self.taxes):
                df.set("print_hide", 0)
            ...
            if is_paid:
                ...
                if cint(self.is_return) and self.grand_total > self.paid_amount:
                    self.paid_amount = flt(
                        flt(self.grand_total), self.precision("paid_amount"))

                elif not flt(self.paid_amount) and flt(self.outstanding_amount) > 0:
                    self.paid_amount = flt(
                        flt(self.outstanding_amount), self.precision("paid_amount"))

                self.base_paid_amount = flt(
                    self.paid_amount * self.conversion_rate,
            ...
            if item.get(item_field) == self.get(party_account_field):
                frappe.throw(
                    _("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
                        item.idx, frappe.bold(
                            frappe.unscrub(item_field)), item.get(item_field), frappe.bold(
                            frappe.unscrub(party_account_field)), self.get(party_account_field), ))
    ...
    def disable_pricing_rule_on_internal_transfer(self):
        if not self.get("ignore_pricing_rule") and self.is_internal_transfer():
            ...
            frappe.msgprint(
                _("Disabled pricing rules since this {} is an internal transfer").format(
                    self.doctype), alert=1, )
        ...
        if self.meta.get_field("currency"):
            ...
            if self.meta.get_field(fieldname) and self.get(fieldname):
                self.price_list_currency = frappe.db.get_value(
                    "Price List", self.get(fieldname), "currency")
            ...
            for item in self.get("items"):
                if item.get("item_code"):
                    ...
                    args["ignore_pricing_rule"] = (
                        self.ignore_pricing_rule if hasattr(
                            self, "ignore_pricing_rule") else 0)
                    ...
                    ret = get_item_details(
                        args, self, for_validate=True, overwrite_warehouse=False)

                    for fieldname, value in ret.items():
                        if item.meta.get_field(
                                fieldname) and value is not None:
                            if item.get(
                                    fieldname) is None or fieldname in force_item_fields:
                                ...
                                # Ensure that serial numbers are matched
                                # against Stock UOM
                                item_conversion_factor = item.get(
                                    "conversion_factor") or 1.0
                                item_qty = abs(
                                    item.get("qty")) * item_conversion_factor
                                ...
                                # reset pricing rule fields if
                                item.set(fieldname, value)

                    if self.doctype in [
                        "Purchase Invoice",
                            "Sales Invoice"] and item.meta.get_field("is_fixed_asset"):
                        item.set(
                            "is_fixed_asset", ret.get(
                                "is_fixed_asset", 0))

                    # Double check for cost center
                    # Items add via promotional scheme may not have cost center
                    if hasattr(item, "cost_center") and not item.get(
                            "cost_center"):
                        item.set(
                            "cost_center",
                            self.get("cost_center") or erpnext.get_default_cost_center(
                                self.company))
                    ...
                    item.conversion_factor = (
                        get_uom_conv_factor(
                            item.get("uom"),
                            item.get("stock_uom")) or item.get("conversion_factor") or 1)
    ...
    def apply_pricing_rule_on_items(self, item, pricing_rule_args):
        if not pricing_rule_args.get("validate_applied_rule", 0):
            # if user changed the discount percentage then set user's discount
            # percentage ?
            if pricing_rule_args.get("price_or_product_discount") == "Price":
                item.set(
                    "pricing_rules",
                    pricing_rule_args.get("pricing_rules"))
                if pricing_rule_args.get("apply_rule_on_other_items"):
                    other_items = json.loads(
                        pricing_rule_args.get("apply_rule_on_other_items"))
                    if other_items and item.item_code not in other_items:
                        return

                item.set("discount_percentage",
                         pricing_rule_args.get("discount_percentage"))
                item.set(
                    "discount_amount",
                    pricing_rule_args.get("discount_amount"))
                if pricing_rule_args.get("pricing_rule_for") == "Rate":
                    item.set("price_list_rate",
                             pricing_rule_args.get("price_list_rate"))
                ...
                if item.get(
                        "apply_discount_on_discounted_rate") and pricing_rule_args.get("rate"):
                    item.rate = pricing_rule_args.get("rate")

            elif pricing_rule_args.get("free_item_data"):
                apply_pricing_rule_for_free_items(
                    self, pricing_rule_args.get("free_item_data"))

        elif pricing_rule_args.get("validate_applied_rule"):
            for pricing_rule in get_applied_pricing_rules(
                    item.get("pricing_rules")):
                pricing_rule_doc = frappe.get_cached_doc(
                    ...
                    if item.get(field) < pricing_rule_doc.get(field):
                        ...
                        frappe.msgprint(
                            _("Row {0}: user has not applied the rule {1} on the item {2}").format(
                                item.idx, frappe.bold(title), frappe.bold(
                                    item.item_code)))
    ...
    def set_taxes(self):
        ...
        if (self.is_new() or self.is_pos_profile_changed()
                ) and not self.get("taxes"):
    ...
    def is_pos_profile_changed(self):
        if (self.doctype == "Sales Invoice" and self.is_pos and self.pos_profile !=
                frappe.db.get_value("Sales Invoice", self.name, "pos_profile")):
            return True

    def append_taxes_from_master(self, tax_master_doctype=None):
        if self.get("taxes_and_charges"):
            if not tax_master_doctype:
                tax_master_doctype = self.meta.get_field(
                    "taxes_and_charges").options

            self.extend(
                "taxes",
                get_taxes_and_charges(
                    tax_master_doctype,
                    self.get("taxes_and_charges")))
        ...
        if self.taxes_and_charges and frappe.get_cached_value(
            ...
            frappe.throw(
                _("{0} '{1}' is disabled").format(
                    taxes_and_charges_doctype,
                    self.taxes_and_charges))

    def validate_tax_account_company(self):
        for d in self.get("taxes"):
            if d.account_head:
                tax_account_company = frappe.get_cached_value(
                    "Account", d.account_head, "company")
        ...
        gl_dict.update(
            {
                ...
                "transaction_exchange_rate": self.get(
                    "conversion_rate",
                    1),
    ...
    def set_advances(self):
        ...
        res = self.get_advance_entries(
            include_unallocated=not cint(
                self.get("only_include_allocated_payments")))
    ...
    def validate_advance_entries(self):
        ...
        if advance_entries:
            advance_entries_against_si = [
                d.reference_name for d in self.get("advances")]
            for d in advance_entries:
                if not advance_entries_against_si or d.reference_name not in advance_entries_against_si:
                    frappe.msgprint(
                        _("Payment Entry {0} is linked against Order {1}, check if it should be pulled as advance in this invoice.").format(
                            d.reference_name, d.against_order))
    ...
    def make_precision_loss_gl_entry(self, gl_entries):
        ...
        precision_loss = self.get("base_net_total") - flt(
            self.get("net_total") * self.conversion_rate,
    ...
    def make_exchange_gain_loss_journal(self, args: dict = None) -> None:
        ...
        if self.docstatus == 1:
            if self.get("doctype") == "Journal Entry":
                ...
                if args:
                    for arg in args:
                        # Advance section uses `exchange_gain_loss` and
                        # reconciliation uses `difference_amount`
                        if (arg.get("difference_amount", 0) != 0 or arg.get(
                                "exchange_gain_loss", 0) != 0) and arg.get("difference_account"):

                            party_account = arg.get("account")
                            gain_loss_account = arg.get("difference_account")
                            difference_amount = arg.get(
                                "difference_amount") or arg.get("exchange_gain_loss")
                            if difference_amount > 0:
                                dr_or_cr = "debit" if arg.get(
                                    "party_type") == "Customer" else "credit"
                            else:
                                dr_or_cr = "credit" if arg.get(
                                    "party_type") == "Customer" else "debit"
            ...
            if self.get("doctype") == "Payment Entry":
                # For Payment Entry, exchange_gain_loss field in the
                # `references` table is the trigger for journal creation
                gain_loss_to_book = [
                    x for x in self.references if x.exchange_gain_loss != 0]
                booked = []
                if gain_loss_to_book:
                    ...
                    if parents:
                        booked = (
                            qb.from_(je) .inner_join(jea) .on(
                                je.name == jea.parent) .select(
                                jea.reference_type,
                                jea.reference_name,
                                jea.reference_detail_no) .where(
                                (je.docstatus == 1) & (
                                    je.name.isin(parents)) & (
                                    je.voucher_type == "Exchange Gain or Loss")) .run())

                for d in gain_loss_to_book:
                    # Filter out References for which Gain/Loss is already
                    if d.exchange_gain_loss and (
                        (d.reference_doctype, d.reference_name, str(
                            d.idx)) not in booked):
    ...
    def unlink_ref_doc_from_po(self):
        ...
        if linked_po:
            frappe.db.set_value(
                "Purchase Order Item", {
                    "sales_order": self.name, "sales_order_item": [
                        "in", so_items], "docstatus": [
                        "<", 2]}, {
                    "sales_order": None, "sales_order_item": None}, )

            frappe.msgprint(
                _("Purchase Orders {0} are un-linked").format("\n".join(linked_po)))
    ...
    def make_discount_gl_entries(self, gl_entries):
        ...
        if enable_discount_accounting:
            for item in self.get("items"):
                if item.get("discount_amount") and item.get(
                        "discount_account"):
    ...
    def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
        ...
        reference_names = [d.get(item_ref_dn)
                           for d in self.get("items") if d.get(item_ref_dn)]
        ...
        for item in self.get("items"):
            ...
            ref_amt = flt(
                reference_details.get(
                    item.get(item_ref_dn)), self.precision(
                    based_on, item))

            if not ref_amt:
                frappe.msgprint(
                    _("System will not check over billing since amount for Item {0} in {1} is zero").format(
                        item.item_code, ref_dt), title=_("Warning"), indicator="orange", )
                continue

            already_billed = self.get_billed_amount_for_item(
                item, item_ref_dn, based_on)
            ...
            if total_billed_amt < 0 and max_allowed_amt < 0:
                # while making debit note against purchase return
                # entry(purchase receipt) getting overbill error
    ...
    def get_stock_items(self):
        ...
        if item_codes:
            stock_items = frappe.db.get_values(
                "Item", {
                    "name": [
                        "in", item_codes], "is_stock_item": 1}, pluck="name", cache=True)
    ...
    def set_total_advance_paid(self):
        ...
        advance = (
            frappe.qb.from_(ple) .select(
                ple.account_currency, Abs(
                    Sum(
                        ple.amount_in_account_currency)).as_("amount")) .where(
                (ple.against_voucher_type == self.doctype) & (
                    ple.against_voucher_no == self.name) & (
                    ple.party == party) & (
                    ple.docstatus == 1) & (
                    ple.company == self.company)) .run(
                as_dict=True))
        ...
        if advance:
            ...
            formatted_advance_paid = fmt_money(
                advance_paid,
                precision=self.precision("advance_paid"),
            ...
            if advance.account_currency == self.currency:
                ...
                precision = "rounded_total" if self.get(
                    "rounded_total") else "grand_total"
            else:
                order_total = self.get(
                    "base_rounded_total") or self.base_grand_total
                precision = "base_rounded_total" if self.get(
                    "base_rounded_total") else "base_grand_total"
    ...
    def company_abbr(self):
        if not hasattr(self, "_abbr"):
            self._abbr = frappe.get_cached_value(
    ...
    def raise_missing_debit_credit_account_error(self, party_type, party):
        ...
        db_or_cr = (frappe.bold("Debit To") if self.doctype ==
                    "Sales Invoice" else frappe.bold("Credit To"))
        rec_or_pay = "Receivable" if self.doctype == "Sales Invoice" else "Payable"

        link_to_party = frappe.utils.get_link_to_form(party_type, party)
        link_to_company = frappe.utils.get_link_to_form(
            "Company", self.company)
        ...
        message += (
            "<li>" +
            _("'Default {0} Account' in Company {1}").format(
                rec_or_pay,
                link_to_company) +
            "</li></ul>")
        ...
        frappe.throw(
            message(
                title=_("Account Missing")).with_traceback(
                exc=AccountMissingError))
                ...
                if (
                    ...
                    frappe.throw(
                        _("Accounting Entry for {0}: {1} can only be made in currency: {2}").format(
                            party_type, party, party_account_currency), InvalidCurrency, )

                # Note: not validating with gle account because we don't have the account
                # at quotation / sales order level and we shouldn't stop someone
                # from creating a sales invoice if sales order is already
    ...
    def validate_party_account_currency(self):
        ...
        party_type, party = self.get_party()
        party_gle_currency = get_party_gle_currency(
            party_type, party, self.company)
        party_account = (self.get("debit_to") if self.doctype ==
                         "Sales Invoice" else self.get("credit_to"))
    ...
    def delink_advance_entries(self, linked_doc_name):
        ...
        for adv in self.advances:
            ...
            if consider_for_total_advance:
                total_allocated_amount += flt(adv.allocated_amount,
    ...
    def group_similar_items(self):
        ...
        for item in self.items:
            group_item_qty[item.item_code] = group_item_qty.get(
                item.item_code, 0) + item.qty
            group_item_amount[item.item_code] = group_item_amount.get(
                item.item_code, 0) + item.amount
        ...
        for item in self.items:
            if item.item_code in group_item_qty:
                ...
                if item.qty:
                    item.rate = flt(flt(item.amount) /
                                    flt(item.qty), item.precision("rate"))
                else:
                    item.rate = 0
    ...
    def set_payment_schedule(self):
        ...
        posting_date = self.get("bill_date") or self.get(
            "posting_date") or self.get("transaction_date")
        date = self.get("due_date")
        due_date = date or posting_date

        base_grand_total = self.get(
            "base_rounded_total") or self.base_grand_total
        ...
        if self.get("total_advance"):
            if party_account_currency == self.company_currency:
                ...
                base_grand_total = flt(
                    grand_total * self.get("conversion_rate"),
    ...
    def set_due_date(self):
        due_dates = [d.due_date for d in self.get(
            "payment_schedule") if d.due_date]
        if due_dates:
    ...
    def validate_payment_schedule_dates(self):
        ...
        for d in self.get("payment_schedule"):
            if self.doctype == "Sales Order" and getdate(
                    d.due_date) < getdate(self.transaction_date):
                frappe.throw(
        ...
        if self.get("payment_schedule"):
            ...
            if self.get("total_advance"):
                if party_account_currency == self.company_currency:
                    base_grand_total -= self.get("total_advance")
                    grand_total = flt(
                        base_grand_total / self.get("conversion_rate"),
                        self.precision("grand_total"))
                else:
                    grand_total -= self.get("total_advance")
                    base_grand_total = flt(
            ...
            if (flt(total,
                    self.precision("grand_total")) - flt(grand_total,
                                                         self.precision("grand_total")) > 0.1 or flt(base_total,
                                                                                                     self.precision("base_grand_total")) - flt(base_grand_total,
                                                                                                                                               self.precision("base_grand_total")) > 0.1):
                frappe.throw(
    ...
    def is_internal_transfer(self):
        ...
        if self.get(internal_party_field) and (
                self.represents_company == self.company):
            return True
    ...
    def create_advance_and_reconcile(self, party_link):
        ...
        primary_account = get_party_account(
            primary_party_type, primary_party, self.company)
        secondary_account = get_party_account(
            secondary_party_type, secondary_party, self.company)
        ...
        advance_entry.cost_center = self.cost_center or erpnext.get_default_cost_center(
            self.company)
    ...
    def check_finance_books(self, item, asset):
        if (
            ...
            frappe.throw(
                _("Select finance book for the item {0} at row {1}").format(
...
def get_default_taxes_and_charges(
    ...
    if tax_template and company:
        tax_template_company = frappe.get_cached_value(
...
def validate_taxes_and_charges(tax):
    if tax.charge_type in [
        ...
        if cint(tax.idx) == 1:
            ...
            frappe.throw(
                _("Please specify a valid Row ID for row {0} in table {1}").format(
                    tax.idx, _(
                        tax.doctype)))
...
def validate_cost_center(tax, doc):
    ...
    company = frappe.get_cached_value(
        "Cost Center", tax.cost_center, "company")

    if company != doc.company:
        frappe.throw(
            _("Row {0}: Cost Center {1} does not belong to Company {2}").format(
                tax.idx, frappe.bold(
                    tax.cost_center), frappe.bold(
                    doc.company)), title=_("Invalid Cost Center"), )
...
def validate_inclusive_tax(tax, doc):
    ...
    if cint(getattr(tax, "included_in_print_rate", None)):
        if tax.charge_type == "Actual":
            ...
            frappe.throw(
                _("Valuation type charges can not be marked as Inclusive"))
...
def get_advance_journal_entries(
    ...
    if include_unallocated:
        q = q.where((journal_acc.reference_name.isnull())
                    | (journal_acc.reference_name == ""))

    if order_list:
        q = q.where((journal_acc.reference_type == order_doctype)
                    & ((journal_acc.reference_type).isin(order_list)))
...
def get_advance_payment_entries(
    ...
    if order_list or against_all_orders:
        ...
        q = q.inner_join(payment_ref).on(
            payment_entry.name == payment_ref.parent)
        q = q.select(
...
def update_invoice_status():
    ...
    for doctype in ("Sales Invoice", "Purchase Invoice"):
        ...
        payable_amount = (
            frappe.qb.from_(payment_schedule) .select(
                Sum(payment_amount)) .where(
        ...
        total_amount = frappe.qb.terms.Case().when(
            consider_base_amount, base_total).else_(total)
...
def get_payment_term_details(
    ...
    term_details.payment_amount = flt(
        term.invoice_portion) * flt(grand_total) / 100
    term_details.base_payment_amount = flt(
        term.invoice_portion) * flt(base_grand_total) / 100
...
def get_discount_date(term, posting_date=None, bill_date=None):
    ...
    if term.discount_validity_based_on == "Day(s) after invoice date":
        ...
        discount_validity = add_days(
            get_last_day(date), term.discount_validity)
    elif term.discount_validity_based_on == "Month(s) after the end of the invoice month":
        discount_validity = get_last_day(
            add_months(date, term.discount_validity))
...
def set_child_tax_template_and_map(item, child_item, parent_doc):
    ...
    if child_item.get("item_tax_template"):
        child_item.item_tax_rate = get_item_tax_map(parent_doc.get(
            "company"), child_item.item_tax_template, as_json=True)
...
def set_order_defaults(
    ...
    child_item = frappe.new_doc(
        child_doctype,
        parent_doc=p_doc,
        parentfield=child_docname)
    ...
    child_item.update({date_fieldname: trans_item.get(
        date_fieldname) or p_doc.get(date_fieldname)})
    child_item.stock_uom = item.stock_uom
    child_item.uom = trans_item.get("uom") or item.stock_uom
    child_item.warehouse = get_item_warehouse(
        item, p_doc, overwrite_warehouse=True)
    conversion_factor = flt(
        get_conversion_factor(
            item.item_code,
            child_item.uom).get("conversion_factor"))
    child_item.conversion_factor = flt(
        trans_item.get("conversion_factor")) or conversion_factor
        ...
        if not child_item.warehouse:
            frappe.throw(
                _("Cannot find {} for item {}. Please set the same in Item Master or Stock Settings.").format(
                    frappe.bold("default warehouse"), frappe.bold(
                        item.item_code)))
...
def update_bin_on_delete(row, doctype):
    ...
    if doctype == "Sales Order":
        qty_dict["reserved_qty"] = get_reserved_qty(
            row.item_code, row.warehouse)
    else:
        if row.material_request_item:
            qty_dict["indented_qty"] = get_indented_qty(
                row.item_code, row.warehouse)
...
def update_child_qty_rate(
    ...
    def validate_quantity(child_item, new_data):
        ...
        if parent_doctype == "Sales Order" and flt(
                new_data.get("qty")) < flt(
                child_item.delivered_qty):
            frappe.throw(_("Cannot set quantity less than delivered quantity"))

        if parent_doctype == "Purchase Order" and flt(
                new_data.get("qty")) < flt(
    ...
    def should_update_supplied_items(doc) -> bool:
        ...
        if update_supplied_items and supplied_items_processed:
            frappe.throw(
                _("Item qty can not be updated as raw materials are already processed."))
        ...
        if is_new:
            if not new_data.get("fg_item"):
                ...
                if not is_sub_contracted_item:
                    ...
                    frappe.throw(
                        _("Default BOM not found for FG Item {0}").format(
                            new_data["fg_item"]))

        if not new_data.get("fg_item_qty"):
            frappe.throw(
                _("Finished Good Item {0} Qty can not be zero").format(
                    new_data["fg_item"]))

    data = json.loads(trans_items)

    any_qty_changed = False  # updated to true if any item's qty changes
    # updated to true if any new item is added or removed
    ...
    for d in data:
        ...
        if not d.get("docname"):
            ...
            child_item = frappe.get_doc(
                parent_doctype + " Item", d.get("docname"))

            prev_rate, new_rate = flt(
                child_item.get("rate")), flt(
                d.get("rate"))
            prev_qty, new_qty = flt(child_item.get("qty")), flt(d.get("qty"))
            prev_fg_qty, new_fg_qty = flt(
                child_item.get("fg_item_qty")), flt(
                d.get("fg_item_qty"))
            ...
            if parent_doctype == "Sales Order":
                prev_date, new_date = child_item.get(
        ...
        if flt(child_item.billed_amt,
               rate_precision) > flt(flt(d.get("rate"),
                                         rate_precision) * flt(d.get("qty"),
                                                               qty_precision),
                                     rate_precision):
            frappe.throw(
        ...
        if d.get("conversion_factor"):
            if child_item.stock_uom == child_item.uom:
                ...
                child_item.conversion_factor = flt(
                    d.get("conversion_factor"), conv_fac_precision)
    ...
    if parent_doctype == "Sales Order":
        from erpnext.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
            cancel_stock_reservation_entries, has_reserved_stock, )
