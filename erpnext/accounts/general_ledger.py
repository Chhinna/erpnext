    ...
    for gle in gl_map:
        for dimension in accounting_dimensions_to_offset:
            ...
            debit = flt(gle.credit) / \
                no_of_dimensions if gle.credit != 0 else 0
            ...
            offsetting_entry.update(
                {
                    ...
                    "remarks": _("Offsetting for Accounting Dimension") + " -\
 {0}".format(dimension.name),
...
def validate_disabled_accounts(gl_map):
    ...
    if disabled_accounts:
        account_list = "<br>"
        account_list += ", ".join([frappe.bold(d.name)
                                  for d in disabled_accounts])
        frappe.throw(_("Cannot create accounting entries against disabled accounts: {0}").format(
            account_list), title=_("Disabled Account Selected"), )
...
def process_gl_map(gl_map, merge_entries=True, precision=None):
    ...
    if gl_map[0].voucher_type != "Period Closing Voucher":
        gl_map = distribute_gl_based_on_cost_center_allocation(
            gl_map, precision)
...
def distribute_gl_based_on_cost_center_allocation(gl_map, precision=None):
    ...
    for d in gl_map:
        ...
        validate_expense_against_budget(d, expense_amount=flt(
            d.debit, precision) - flt(d.credit, precision))

        if cost_center and cost_center_allocation.get(cost_center):
            for sub_cost_center, percentage in cost_center_allocation.get(
                    cost_center, {}).items():
                gle = copy.deepcopy(d)
                ...
                for field in (
                    ...
                    gle[field] = flt(flt(d.get(field)) *
                                     percentage / 100, precision)
...
def get_cost_center_allocation_data(company, posting_date):
    ...
    for d in records:
        cc_allocation.setdefault(
            d.main_cost_center,
            frappe._dict()).setdefault(
            d.cost_center,
            d.percentage)
...
def merge_similar_entries(gl_map, precision=None):
    ...
    for entry in gl_map:
        ...
        if same_head:
            ...
            same_head.debit_in_account_currency = flt(
                same_head.debit_in_account_currency) + flt(entry.debit_in_account_currency)
            same_head.credit = flt(same_head.credit) + flt(entry.credit)
            same_head.credit_in_account_currency = flt(
                same_head.credit_in_account_currency) + flt(entry.credit_in_account_currency)
        else:
    ...
    merged_gl_map = filter(
        lambda x: flt(
            ...
            x.credit,
            precision) != 0 or (
                x.voucher_type == "Journal Entry" and frappe.get_cached_value(
                    "Journal Entry",
                    x.voucher_no,
    ...
    for entry in gl_map:
        ...
        if flt(entry.debit_in_account_currency) < 0:
            entry.credit_in_account_currency = flt(
                entry.credit_in_account_currency) - flt(entry.debit_in_account_currency)
...
def process_debit_credit_difference(gl_map):
    ...
    if abs(debit_credit_diff) > allowance:
        if not (voucher_type == "Journal Entry" and frappe.get_cached_value(
                "Journal Entry", voucher_no, "voucher_type") == "Exchange Gain Or Loss"):
...
def raise_debit_credit_not_equal_error(
        debit_credit_diff, voucher_type, voucher_no):
    frappe.throw(
...
def make_round_off_gle(gl_map, debit_credit_diff, precision):
    ...
    if gl_map[0].voucher_type != "Period Closing Voucher":
        ...
        if round_off_account_exists and abs(
                debit_credit_diff) < (1.0 / (10**precision)):
...
def make_reverse_gl_entries(
    ...
    if gl_entries:
        ...
        if not partial_cancel:
            set_as_cancel(
                gl_entries[0]["voucher_type"],
                gl_entries[0]["voucher_no"])
        ...
        for entry in gl_entries:
            ...
            debit_in_account_currency = new_gle.get(
                "debit_in_account_currency", 0)
            credit_in_account_currency = new_gle.get(
...
def validate_against_pcv(is_opening, posting_date, company):
    ...
    last_pcv_date = frappe.db.get_value(
        "Period Closing Voucher", {
            "docstatus": 1, "company": company}, "max(posting_date)")
