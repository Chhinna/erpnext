...
def _get_party_details(
    ...
    if not ignore_permissions and not (
        ...
        frappe.throw(
            _("Not permitted for {0}").format(party)(
                frappe.PermissionError))

    party = frappe.get_doc(party_type, party)
    currency = party.get(
        "default_currency") or currency or get_company_currency(company)
    ...
    if not party_details.get("tax_category") and pos_profile:
        party_details["tax_category"] = frappe.get_value(
            "POS Profile", pos_profile, "tax_category")
...
def set_address_details(
    ...
    billing_address_field = (
        "customer_address" if party_type == "Lead" else party_type.lower() +
        "_address")
    ...
    if doctype in PURCHASE_TRANSACTION_TYPES:
        ...
        if party_details.company_address:
            # billing address
            party_details.update(
                billing_address=party_details.company_address, billing_address_display=(
                    party_details.company_address_display or get_address_display(
                        party_details.company_address)), **get_fetch_values(
                    doctype, "billing_address", party_details.company_address))
...
def set_other_values(party_details, party, party_type):
    ...
    for f in ["currency"] + (["sales_partner", "commission_rate"]
...
def set_price_list(
    ...
    if price_list and len(price_list) == 1:
        ...
        customer_price_list = frappe.get_value(
            "Customer", party.name, "default_price_list")

        if customer_price_list:
            price_list = customer_price_list
        else:
            pos_price_list = frappe.get_value(
                "POS Profile", pos, "selling_price_list")
...
def get_party_account(
    ...
    if not party and party_type in ["Customer", "Supplier"]:
        ...
        return frappe.get_cached_value(
            "Company", company, default_account_name)
    ...
    if not account and party_type in ["Customer", "Supplier"]:
        ...
        group = frappe.get_cached_value(
            party_type, party, scrub(party_group_doctype))
        account = frappe.db.get_value(
            "Party Account",
    ...
    if existing_gle_currency:
        ...
        if (account and account_currency !=
                existing_gle_currency) or not account:
...
def validate_party_gle_currency(
        party_type,
        party,
        company,
...
def validate_party_accounts(doc):
    ...
    for account in doc.get("accounts"):
        if account.company in companies:
            frappe.throw(
                _("There can only be 1 Account per Company in {0} {1}").format(
                    doc.doctype, doc.name), DuplicatePartyAccountError, )
        else:
            companies.append(account.company)

        party_account_currency = frappe.get_cached_value(
            "Account", account.account, "account_currency")
        ...
        if doc.get(
                "default_currency") and party_account_currency and company_default_currency:
            if (
                    doc.default_currency != party_account_currency
                    and doc.default_currency != company_default_currency
            ):
                frappe.throw(
                    _("Billing currency must be equal to either default company's currency or party account currency"))
    ...
    if (bill_date or posting_date) and party:
        ...
        if template_name:
            ...
            if party_type == "Supplier":
                supplier_group = frappe.get_cached_value(
                    party_type, party, "supplier_group")
                template_name = frappe.get_cached_value(
                    "Supplier Group", supplier_group, "payment_terms")
                if template_name:
                    due_date = get_due_date_from_template(
                        template_name, posting_date, bill_date).strftime("%Y-%m-%d")
...
def validate_due_date(
    ...
    if getdate(due_date) < getdate(posting_date):
        ...
        if default_due_date != posting_date and getdate(
                due_date) > getdate(default_due_date):
    ...
    if addr_tax_category_from == "Shipping Address":
        if shipping_address:
            tax_category = frappe.db.get_value(
                "Address", shipping_address, "tax_category") or tax_category
    else:
        if billing_address:
            tax_category = frappe.db.get_value(
                "Address", billing_address, "tax_category") or tax_category
...
def get_payment_terms_template(party_name, party_type, company=None):
    ...
    if party_type == "Customer":
        customer = frappe.get_cached_value(
            "Customer", party_name, fieldname=[
                "payment_terms", "customer_group"], as_dict=1)
        template = customer.payment_terms

        if not template and customer.customer_group:
            template = frappe.get_cached_value(
                "Customer Group", customer.customer_group, "payment_terms")
    else:
        supplier = frappe.get_cached_value(
            "Supplier", party_name, fieldname=[
                "payment_terms", "supplier_group"], as_dict=1)
        template = supplier.payment_terms
        if not template and supplier.supplier_group:
            template = frappe.get_cached_value(
                "Supplier Group", supplier.supplier_group, "payment_terms")

    if not template and company:
        template = frappe.get_cached_value(
            "Company", company, fieldname="payment_terms")
    return template
...
def validate_party_frozen_disabled(party_type, party_name):
    ...
    if party_type and party_name:
        if party_type in ("Customer", "Supplier"):
            party = frappe.get_cached_value(
                party_type, party_name, [
                    "is_frozen", "disabled"], as_dict=True)
            if party.disabled:
                frappe.throw(
                    _("{0} {1} is disabled").format(
                        party_type, party_name)(PartyDisabled))
                ...
                if frozen_accounts_modifier not in frappe.get_roles():
                    frappe.throw(
                        _("{0} {1} is frozen").format(
                            party_type, party_name)(PartyFrozen))
            ...
            if frappe.db.get_value(
                ...
                party_name,
                    "status") != "Active":
                frappe.msgprint(
                    _("{0} {1} is not active").format(
                        party_type, party_name), alert=True)
...
def get_dashboard_info(party_type, party, loyalty_program=None):
    ...
    for d in company_wise_grand_total:
        company_wise_billing_this_year.setdefault(
            d.company, {
                "grand_total": d.grand_total, "base_grand_total": d.base_grand_total})
    ...
    for d in companies:
        company_default_currency = frappe.get_cached_value(
            "Company", d.company, "default_currency")
        party_account_currency = get_party_account_currency(
            party_type, party, d.company)

        if party_account_currency == company_default_currency:
            billing_this_year = flt(
                company_wise_billing_this_year.get(
                    d.company, {}).get("base_grand_total"))
        else:
            billing_this_year = flt(
                company_wise_billing_this_year.get(
                    d.company, {}).get("grand_total"))
